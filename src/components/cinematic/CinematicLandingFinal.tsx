...
const rooms = [
  {
    ...
    position: [0, 1.25, 8] as [number, number, number],
  ...
  },
  {
    ...
    position: [-3.1, 1.15, 3.8] as [number, number, number],
    look: [-1.2, 1.1, 2] as [number, number, number],
  },
  {
    name: "Living Room",
    eyebrow: "03",
    subtitle: "The heart of everyday life",
    position: [-3.2, 1.15, 0] as [number, number, number],
    look: [-1, 1.1, -1.7] as [number, number, number],
  },
  {
    name: "Kitchen",
    eyebrow: "04",
    subtitle: "Function, flow and material intelligence",
    position: [3, 1.2, -1.8] as [number, number, number],
    look: [1.5, 1.05, -3.2] as [number, number, number],
  },
  {
    name: "Master Bedroom",
    eyebrow: "05",
    subtitle: "Quiet, warm and deeply personal",
    position: [3, 1.2, 2.1] as [number, number, number],
    look: [1.2, 1.05, 0.6] as [number, number, number],
  },
  {
    name: "Kids' Room",
    eyebrow: "06",
    subtitle: "Playful, practical and ready to grow",
    position: [0.4, 1.2, -5.2] as [number, number, number],
    look: [0.4, 1.05, -7] as [number, number, number],
  },
  {
    name: "Balcony",
    eyebrow: "07",
    subtitle: "The final frame: open sky and home",
    position: [-2.7, 1.3, -9] as [number, number, number],
    look: [-2.7, 1.15, -10.5] as [number, number, number],
  },
];
    ...
    <group>
      ...
      {tiles.map((i) => (
        <mesh
          key={i}
          rotation={[-Math.PI / 2, 0, 0]}
          position={[-4.25 + (i % 6) * 1.7, 0.01, 6 - Math.floor(i / 6) * 3.9]}
          receiveShadow
        >
          <planeGeometry args={[1.55, 3.6]} />
          <meshStandardMaterial
            color={i % 2 ? "#b5aa99" : "#9e9382"}
            roughness={0.78}
          />
        </mesh>
      ))}
...
function WalkthroughScene({
  ...
  useFrame((_, delta) => {
    ...
    const progress = THREE.MathUtils.clamp(
      (inSegment - pauseSeconds) / travelSeconds,
      0,
      1,
    );
...
function CinematicHero({ imageUrl }: { imageUrl: string }) {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[#12110f]">
      <div
        className={`absolute inset-0 transition-transform duration-[4000ms] ease-out ${loaded ? "scale-100" : "scale-[1.035]"}`}
      >
        <div
      ...
      <header className="relative z-10 flex items-center justify-between px-5 py-6 md:px-12 md:py-8">
        ...
        <a
          href="#walkthrough"
          className="rounded-full border border-white/20 bg-black/20 px-4 py-2 font-body text-xs text-white/85 backdrop-blur-md"
        >
          Enter the home
        </a>
      </header>
      <div className="relative z-10 mx-auto flex min-h-[88svh] max-w-7xl items-end px-5 pb-14 md:px-12 md:pb-24">
        <div className="max-w-5xl">
          <p className="font-mono text-[10px] uppercase tracking-[0.32em] text-[#d7b679]">
            4BHK · NIWASTHAN RESIDENCE
          </p>
          <h1 className="mt-5 font-display text-[clamp(3.2rem,9vw,8.5rem)] leading-[.88] tracking-[-.055em]">
            Your home.
            <br />
            Designed your way.
          </h1>
          <p className="mt-7 max-w-xl font-body text-sm leading-relaxed text-white/70 md:text-base">
      ...
      <div className="absolute bottom-7 left-5 z-10 flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.22em] text-white/50 md:left-12">
...
export function CinematicLandingFinal() {
  ...
  return (
    <main className="min-h-screen bg-[#12110f] text-[#f4eee2] selection:bg-[#d7b679] selection:text-[#171512]">
      ...
      <section
        id="walkthrough"
        className="border-y border-white/10 bg-[#171512] py-14 sm:py-20 md:py-24"
      >
        <div className="mx-auto max-w-7xl px-5 md:px-12">
          <div className="mb-7 flex flex-col gap-5 md:mb-9 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="mt-3 max-w-4xl font-display text-4xl leading-none tracking-[-.04em] sm:text-5xl md:text-7xl">
                Walk through the home.
                <br className="hidden sm:block" /> Feel the spaces.
              </h2>
            </div>
            <p className="max-w-sm font-body text-sm leading-relaxed text-white/50">
              Low camera. Long easing. Material close-ups. A deliberate
              two-second pause in every room.
            </p>
          </div>
          ...
          <div className="relative h-[58svh] min-h-[430px] overflow-hidden rounded-[24px] border border-white/10 bg-black shadow-2xl sm:h-[66vh] sm:min-h-[520px] md:rounded-[30px]">
            ...
            <div className="absolute left-4 top-4 rounded-full border border-white/15 bg-black/30 px-3 py-2 font-mono text-[9px] uppercase tracking-[.18em] text-white/65 backdrop-blur-md sm:left-5 sm:top-5 sm:px-4">
              Real-time 3D · Cinematic camera
            </div>
            <div className="absolute inset-x-4 bottom-4 flex items-end justify-between gap-3 sm:inset-x-6 sm:bottom-6">
              <div className="min-w-0">
                <p className="font-mono text-[9px] uppercase tracking-[.22em] text-[#d7b679]">
                  Now entering · {rooms[activeRoom].eyebrow}
                </p>
                <h3 className="mt-1 truncate font-display text-3xl tracking-[-.03em] sm:text-4xl">
                  {rooms[activeRoom].name}
                </h3>
                <p className="mt-1 hidden font-body text-sm text-white/55 sm:block">
              ...
              <div className="flex shrink-0 gap-2">
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/20 bg-black/45 backdrop-blur-md hover:bg-white/10 sm:h-11 sm:w-11"
                >
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            {rooms.map((room, index) => (
              <button
                ...
                className={`rounded-xl border px-3 py-3 text-left transition ${index === activeRoom ? "border-[#d7b679]/60 bg-[#d7b679]/10" : "border-white/10 bg-white/[.02] hover:bg-white/[.05]"}`}
              >
      ...
      <section className="px-5 py-20 sm:py-24 md:px-12 md:py-32">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-end">
          <div>
            ...
            <h2 className="mt-4 max-w-3xl font-display text-4xl leading-[.95] tracking-[-.04em] sm:text-5xl md:text-7xl">
              See the home.
              <br />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              >
                ...
                <h3 className="mt-5 font-display text-xl tracking-[-.02em]">
                  {title}
                </h3>
