...
const observation = (
  ...
  evidence: {
    ...
    verifiedAt: truth === "VERIFIED" ? new Date("2026-08-29T00:00:00.000Z") : undefined,
  },
  amountMinor,
  currency: "INR",
  available: true,
...
describe("dealIntelligenceService", () => {
  it("compares the exact selected variant and distinguishes verified deals", async () => {
    ...
    vi.spyOn(marketIntelligenceRepository, "listExactVariantObservations").mockResolvedValue([
      observation("deal", "variant-1", 75_000n),
    ]);
