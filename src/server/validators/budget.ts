...
const minorMoney = z.number().int().nonnegative().safe();
...
export const budgetImpactSchema = z.object({
  ...
  proposedLowDeltaMinor: z.number().int().safe(),
  proposedTargetDeltaMinor: z.number().int().safe(),
  proposedHighDeltaMinor: z.number().int().safe(),
