...
class RazorpayProvider implements PaymentProvider {
  constructor(private readonly keyId = process.env.RAZORPAY_KEY_ID, private readonly keySecret = process.env.RAZORPAY_KEY_SECRET, private readonly webhookSecret = process.env.RAZORPAY_WEBHOOK_SECRET) {
  ...
  async createOrder(input: { amountMinor: bigint; currency: string; receipt: string }): Promise<PaymentOrder> {
    ...
    const response = await fetch("https://api.razorpay.com/v1/orders", { method: "POST", headers: { Authorization: `Basic ${auth}`, "Content-Type": "application/json" }, body: JSON.stringify({ amount: Number(input.amountMinor), currency: input.currency, receipt: input.receipt }) });
  ...
  verifyWebhook(rawBody: string, signature: string) {
    return verifyRazorpayWebhookSignature(rawBody, signature, this.webhookSecret!);
  }
}
