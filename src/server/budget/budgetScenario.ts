...
export function calculateRevisionImpact(
  ...
  const revisedMinor = revised.lines.reduce(
    (sum, line) => sum + line.amountMinor,
    0n,
  );
  if (baselineMinor < 0n || revisedMinor < 0n)
  ...
  const revisedById = new Map(
    revised.lines.map((line) => [line.id, line.amountMinor]),
  );
  const ids = new Set([...baselineById.keys(), ...revisedById.keys()]);
  const changedLineIds = [...ids]
    .filter((id) => baselineById.get(id) !== revisedById.get(id))
    .sort();
  return {
    baselineMinor,
    revisedMinor,
