...
export function calculateBudgetScenario(
  ...
  const scenarioMinor = scenario.lines.reduce((sum, line) => sum + line.amountMinor, 0n);
  if (baselineMinor < 0n || scenarioMinor < 0n) throw new Error("budget amounts cannot be negative");
  ...
  const savingsMinor = baselineMinor > scenarioMinor ? baselineMinor - scenarioMinor : 0n;
  const savingsBps = baselineMinor === 0n ? 0 : Number((savingsMinor * 10_000n) / baselineMinor);
  return { baselineMinor, scenarioMinor, savingsMinor, savingsBps, changedLineIds };
}
