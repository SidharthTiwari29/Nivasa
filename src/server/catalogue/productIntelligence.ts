...
export const normaliseProductCandidate = (
...
export const matchesProductConstraint = (
  ...
  if (
    constraint.category &&
    product.category !== normalise(constraint.category)
  )
    return false;
  if (constraint.unit && product.unit !== normalise(constraint.unit))
    return false;
  if (
    constraint.maxPriceMinor !== undefined &&
    (product.priceMinor === undefined ||
      product.priceMinor > constraint.maxPriceMinor)
  )
    return false;
...
export const rankProductCandidates = (
  ...
  candidates
    ...
    .sort((left, right) => {
      ...
      const evidenceDelta =
        (quality[right.evidenceQuality ?? "UNKNOWN"] ?? 0) -
