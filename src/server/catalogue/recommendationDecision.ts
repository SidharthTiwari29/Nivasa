...
export function chooseRecommendation(
  ...
  const eligible = candidates.filter(
    (candidate) =>
      candidate.evidenceBacked &&
      candidate.priceMinor >= 0n &&
      Number.isFinite(candidate.score),
  );
  if (!eligible.length)
    throw new Error("no evidence-backed recommendation candidates");
  ...
  return {
    ...
    mode,
    reason: `selected by ${mode.toLowerCase()} from evidence-backed candidates`,
  };
}
