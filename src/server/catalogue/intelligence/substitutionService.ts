...
export type SubstitutionReason =
  "LOWER_COST" | "UNAVAILABLE" | "BETTER_CONFIDENCE";
...
export function rankSubstitutions(
  ...
  if (
    selectedObservation.canonicalProductId !==
    selectedVariant.canonicalProductId
  ) {
    throw new Error(
      "selected observation does not belong to selected variant product",
    );
  }
  ...
  return candidates
    ...
    .map(({ variant, observation }) => {
      ...
      if (observation.amountMinor < selectedObservation.amountMinor)
        reasons.push("LOWER_COST");
      if (!selectedObservation.available && observation.available)
        reasons.push("UNAVAILABLE");
      if (observation.confidenceBps > selectedObservation.confidenceBps)
        reasons.push("BETTER_CONFIDENCE");
      const score =
        reasons.length * 10_000 +
        observation.confidenceBps +
        (observation.available ? 1_000 : 0);
