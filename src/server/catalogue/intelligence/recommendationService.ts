...
export function recommend(
  ...
  const maxPrice = candidates.reduce(
    (max, c) =>
      c.observation.amountMinor > max ? c.observation.amountMinor : max,
    0n,
  );
  const maxDays = candidates.reduce(
    (max, c) => Math.max(max, c.signals?.deliveryDays ?? 0),
    0,
  );

  return candidates
    .map(({ observation, signals }) => {
      validateConfidenceBps(observation.confidenceBps);
      const priceBps =
        maxPrice === 0n
          ? 0
          : Number(((maxPrice - observation.amountMinor) * 10_000n) / maxPrice);
      const speedBps =
        maxDays === 0
          ? 0
          : Math.round(
              ((maxDays - (signals?.deliveryDays ?? maxDays)) / maxDays) *
                10_000,
            );
      ...
      const scoreBps = Math.round(
        (priceBps * w.price +
          observation.confidenceBps * w.confidence +
          availabilityBps * w.availability +
          qualityBps * w.quality +
          speedBps * w.speed +
          localBps * w.local) /
          10,
      );
      ...
      if (w.confidence && observation.confidenceBps >= 9_000)
