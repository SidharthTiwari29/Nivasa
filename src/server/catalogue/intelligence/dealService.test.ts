...
const item = (id: string, amountMinor: bigint, available = true): MarketObservation => ({
  observationId: id,
...
describe("findDeals", () => {
  it("returns available options meeting the saving threshold", () => {
    ...
    expect(deals.map((deal) => deal.observation.observationId)).toEqual(["b", "a"]);
  ...
  it("rejects an invalid threshold", () => {
    expect(() => findDeals(100n, [], 10_001)).toThrow("minimumSavingBps must be between 0 and 10000");
  });
});
