...
export const validateIngestionCounters = (counters: IngestionCounters): void => {
  ...
  if (!Number.isInteger(counters.recordsAccepted) || counters.recordsAccepted < 0) {
    throw new Error("recordsAccepted must be a non-negative integer");
  }
  if (!Number.isInteger(counters.recordsRejected) || counters.recordsRejected < 0) {
    throw new Error("recordsRejected must be a non-negative integer");
  }
  if (counters.recordsAccepted + counters.recordsRejected > counters.recordsSeen) {
...
export const summarizeIngestionRun = (input: {
  ...
  const errors = [...new Set((input.errors ?? []).map((error) => error.trim()).filter(Boolean))];
  let status: IngestionStatus = "SUCCEEDED";
  if (input.cancelled) status = "CANCELLED";
  else if (input.counters.recordsRejected > 0 && input.counters.recordsAccepted > 0) status = "PARTIAL";
  else if (input.counters.recordsRejected > 0) status = "FAILED";

  return { ...input.counters, status, errors };
};

export const buildReplayKey = (sourceKey: string, idempotencyKey: string): string => {
  const source = sourceKey.trim();
  const key = idempotencyKey.trim();
  if (!source || !key) throw new Error("sourceKey and idempotencyKey are required");
  return `${source}:${key}`;
};
...
export const calculateSourceHealth = (
  sourceKey: string,
  runs: readonly { status: IngestionStatus; completedAt: Date | null }[],
): SourceHealth => {
  ...
  const latest = (items: typeof matching): Date | null =>
    items.reduce<Date | null>((latestDate, run) => {
      ...
      return !latestDate || run.completedAt > latestDate ? run.completedAt : latestDate;
    }, null);
