...
const qualityScore = (
  quality: RecommendationCandidate["evidenceQuality"],
): number =>
  ({
    HIGH: 30,
    MEDIUM: 20,
...
export const recommendDesignCandidates = (
  ...
  candidates
    .map((candidate) => {
      ...
      if (
        context.requiredCategory &&
        normalise(candidate.category) === normalise(context.requiredCategory)
      ) {
        score += 25;
      ...
      if (
        context.room &&
        candidate.room &&
        normalise(candidate.room) === normalise(context.room)
      ) {
        score += 20;
        reasons.push("matches room");
      }

      const candidateStyles = new Set(
        (candidate.styleTags ?? []).map(normalise),
      );
      const matchedStyles = (context.preferredStyles ?? []).filter((style) =>
        candidateStyles.has(normalise(style)),
      );
      ...
      if (
        context.maxPriceMinor !== undefined &&
        candidate.priceMinor !== undefined &&
        candidate.priceMinor <= context.maxPriceMinor
      ) {
        score += 15;
      ...
      if (reasons.length === 0)
        reasons.push("no specific constraint match; evidence quality only");
