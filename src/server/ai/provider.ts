...
class UnconfiguredAIProvider implements AIProvider {
  private fail(): Promise<never> {
    return Promise.reject(new Error("AI_PROVIDER_NOT_CONFIGURED"));
  }
  analyzeFloorPlan(): Promise<AIResult> {
    return this.fail();
  }
  generateDesign(): Promise<AIResult> {
    return this.fail();
  }
  reviseDesign(): Promise<AIResult> {
    return this.fail();
  }
  assistBoq(): Promise<AIResult> {
    return this.fail();
  }
  createWalkthroughPrompt(): Promise<AIResult> {
    return this.fail();
  }
}
